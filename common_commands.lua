...
function execute_common_command(passedCommand)
	...
	elseif passedCommand:slice(1,6) == 'mirror' then
		...
		return true
    elseif passedCommand:sub(1,12) == "partytarget " then
        local possible_target = passedCommand:gsub("partytarget ","")
        if tonumber(possible_target) ~= nil then
            party_target = tonumber(possible_target)
            if check_party_target_exists() then
                party_target_change()
                return true
            end
        end
...
function check_party_target_exists()
    if party_target == false then
        return false
    end

    local local_mobs = windower.ffxi.get_mob_by_id(party_target)
...
function nearest_claimable_enemies(filterNames)
    local mobs = windower.ffxi.get_mob_array()
    local claimables = T{}
    for id,mob in pairs(mobs) do
        if mob.distance <= 484 then
            if type(filterNames) == 'table' and #filterNames > 0 then
                for i,v in pairs(filterNames) do
                    if (v == mob.name) and is_mob_claimable(mob) then
                        claimables:append(mob)
                    end
                end
            elseif is_mob_claimable(mob) then
                claimables:append(mob)
            end
        end
    end
...
function is_mob_claimable(mob)
    if mob.distance == 0.089004568755627 then
        -- print('Invalid Target - not in range '..mob.distance..' '..mob.name)
        return false
    end
    if not mob then
        -- print('Invalid Target - not in memory '..mob.name)
        return false
    end
    if mob.hpp == 0 then
        -- print('Invalid Target - 0 HP '..mob.name)
        return false
    end
    if not mob.valid_target then
        -- print('Invalid Target - valid_target is false '..mob.name)
        return false
    end
    if mob.in_party then
        -- print('Invalid Target - in party'..mob.name)
        return false
    end
    if mob.in_alliance then
        -- print('Invalid Target - in alliance'..mob.name)
        return false
    end
    if not mob.is_npc then
        -- print('Invalid Target - not NPC'..mob.name)
        return false
    end
    if mob.charmed then
        -- print('Invalid Target - is charmed'..mob.name)
        return false
    end
    return true
end