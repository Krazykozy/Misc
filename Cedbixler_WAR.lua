...
function get_sets()
	...
	WeaponIndex = 1
	WeaponArray = {"Ragnarok","Aganoshe","Tanmogayi","Tramontane","Blurred"} -- Default Main Weapon Is Ragnarok. Can Delete Any Weapons/Sets That You Don't Need Or Replace/Add The New Weapons That You Want To Use. --
	IdleIndex = 1
	...
	sets.Idle.Regen = {
			...
			body={ name="Valorous Mail", augments={'Crit. hit damage +1%','DEX+7','"Store TP"+9','Accuracy+11 Attack+11',}},
	...
	sets.Idle.Regen.Ragnarok = set_combine(sets.Idle.Regen,{
			main={ name="Ragnarok", augments={'Haste+1','STR+13','Accuracy+21','Attack+22','DMG:+27',}},
			sub="Utu Grip"})
	...
	-- Upheaval Sets --
	sets.WS.Upheaval = {
			main={ name="Aganoshe", augments={'"Store TP"+4','DEX+17','Accuracy+12','DMG:+23',}},
			sub="Utu Grip",
			ammo="Iron Gobbet",
			head="Sulevia's Mask +2",
...
function precast(spell,action)
	if spell.type == "WeaponSkill" then
		if player.status ~= 'Engaged' then -- Cancel WS If You Are Not Engaged. Can Delete It If You Don't Need It --
			...
			if player.tp > 2999 or buffactive.Sekkanoki then -- Equip Vulcan's Pearl When You Have 300 TP or Sekkanoki --
				equipSet = set_combine(equipSet,{left_ear="Vulcan's Pearl"})
			end
			if spell.english == "Vorpal Blade" and player.tp > 2999 then
				equipSet = set_combine(equipSet,{left_ear="Vulcan's Pearl"})
			end
			if spell.english == "Ground Strike" and player.tp > 2999 then
				equipSet = set_combine(equipSet,{left_ear="Vulcan's Pearl"})
			end
			if spell.english == "Savage Blade" and player.tp > 2999 then
				equipSet = set_combine(equipSet,{left_ear="Vulcan's Pearl"})
			end
			if spell.english == "Ukko's Fury" and player.tp > 2999 then
				equipSet = set_combine(equipSet,{left_ear="Vulcan's Pearl"})
			end
			if spell.english == "Raging Rush" and player.tp > 2999 then
				equipSet = set_combine(equipSet,{left_ear="Vulcan's Pearl"})
			end
			if spell.english == "Resolution" and player.tp > 2999 then
				equipSet = set_combine(equipSet,{left_ear="Vulcan's Pearl"})
			end
			if spell.english == "Upheaval" and player.tp > 2999 then
