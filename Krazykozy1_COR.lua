...
function handle_idle()
	...
	if player.status ~= 'Engaged' then
        if player.mpp < 60 and (player.sub_job == 'WHM' or player.sub_job == 'RDM' or player.sub_job == 'SCH' or player.sub_job == 'BLM') then
...
function self_command(command)
    ...
    elseif command == 'request_speed' then
        send_command('input /ja "Bolter\'s Roll" <me>;')
	...
	if check_ready_silent() and player.x == lastx and player.y == lasty then
        ...
        if player.status == 'Engaged' and autoShoot:current() then
            ...
            if autoWS:current() and player.tp >= 1000 then
                add_to_back(stack_weapon_skills['Last Stand'], target_to_use, {})
            else
                add_to_back('Ranged Attack', target_to_use, {})
            end
