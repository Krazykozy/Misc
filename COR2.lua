...
autoRoll = false
doCures = false
auto_JA = I{false, true}
auto_RA = I{false, true}
auto_ws = I{false,true}
auto_ws_to_use = I{"Savage Blade","Last Stand", "Leaden Salute", "Requiescat"}
...
function get_sets()
	...
	accMode = I{'LowAcc', 'MidAcc', 'HighAcc'}
	auto_ws_to_use:setTo("Savage Blade")	
...
function midcast(spell,action)
    ...
    elseif spell.english == "Ranged" then
        equip(sets[accMode:current()]['Standard'])
	end
end
...
function self_command(command)
    if command == 'tp' or command == 'dt' then
        tp_mode_type:next()
    ...
    elseif command == 'autora' or command == 'ra' then
        if autoRA then
            autoRA = false
            windower.add_to_chat(167,"Auto RA [OFF]")
        else
            autoRA = true
            windower.add_to_chat(167,"Auto RA [ON]")
        end		
...
function handle_ping()
	...
	if check_ready_silent() and player.x == lastx and player.y == lasty then
        ...
        if player.status == 'Engaged' and finished_rolling_1 == true and finished_rolling_2 == true then
            if auto_ws:current() and player.tp >= 1000 then
				direct_push_front(stack_weapon_skills[auto_ws_to_use:current()], 'ws', 't', false)
            elseif auto_RA:current() then
                add_to_back('Ranged Attack', 't', {})
            else
			end

            if auto_JA:current() then
