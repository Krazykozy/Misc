...
function self_command(command)
	...
	elseif command == 'tank' then
		...
		handle_idle()
    elseif command == 'autosolace' then
        if not autoSolace then
            autoSolace = true
            windower.add_to_chat(028, "Auto Afflatus Solace [ON]")
        else
            autoSolace = false
            windower.add_to_chat(028, "Auto Afflatus Solace [OFF]")
        end
	...
	elseif command == 'debuff' then
        if is_spell_ready('Silence') then
            direct_push(stack_spells['Silence'], 'ma', 'bt', false)
        end
        if is_spell_ready('Slow') then
            direct_push(stack_spells['Slow'], 'ma', 'bt', false)
        end
        if is_spell_ready('Paralyze') then
            direct_push(stack_spells['Paralyze'], 'ma', 'bt', false)
        end
        if is_spell_ready('Addle') then
            direct_push(stack_spells['Addle'], 'ma', 'bt', false)
        end

        if player.sub_job == 'RDM' then
            if is_spell_ready('Frazzle') then
                direct_push(stack_spells['Frazzle'], 'ma', 'bt', false)
            end
            if is_spell_ready('Distract') then
                direct_push(stack_spells['Distract'], 'ma', 'bt', false)
            end
        elseif player.sub_job == 'BLM' then
            if is_spell_ready('Blind') then
                direct_push(stack_spells['Blind'], 'ma', 'bt', false)
            end
            if is_spell_ready('Poison') then
                direct_push(stack_spells['Poison'], 'ma', 'bt', false)
            end
        end
    ...
    elseif command == 'ambu' or command == 'ambu melee' or command == 'ambu mage' or command == 'melee buffs' then
        dtMode = true
        autoSS = false
        autoAquaveil = true

        -- priority_players['Miang'] = 1.1
        -- priority_players['Nekachu'] = 1.1
        -- priority_players['Mazuki'] = 1.0
        -- priority_players['Norzu'] = 1.0
        -- priority_players['Ulor'] = 1.1
        -- priority_players['Tosel'] = 0.5
    ...
    elseif command == 'trials' then
        dtMode = false
        auto_spell_toggle('Nekachu', 'Protectra V', {force_on=true})
        auto_spell_toggle('Nekachu', 'Shellra V', {force_on=true})
        auto_spell_toggle('Nekachu', 'Boost-STR', {force_on=true})
        auto_spell_toggle('Nekachu', 'Auspice', {force_on=true})
    ...
    elseif command == 'omen trash' then
        if not buffactive['protect'] or not buffactive['shell'] then
            process_push_string('firebuffs', 'Nekachu')
        end
...
function handle_ping()
    ...
    if player.mpp < 35 then
        if os.clock()-lastChat > 10 then
            send_command('input /p Running low on mp')
            lastChat = os.clock()
        end
    end
    ...
    if check_ready_silent() and player.x == lastx and player.y == lasty then
		...
		if autoSolace and not buffactive['afflatus solace'] and not buffactive['afflatus misery'] and is_ability_ready('Afflatus Solace')then
